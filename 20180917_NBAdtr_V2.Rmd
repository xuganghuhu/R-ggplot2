...
ggplot(dat_nba1,aes(cut_age,fill=POSITION))+
  geom_bar(position = 'stack')+                        #并列dodge  堆叠stack
...
ggplot(dat_nba3,aes(TEAM,TRB,fill=TEAM))+geom_boxplot()+geom_text(aes(label=PLAYER),color='black',check_overlap = TRUE)+theme_void()

ggplot(dat_nba3,aes(TEAM,RPM,fill=TEAM))+geom_boxplot()+geom_text(aes(label=PLAYER),color='black',check_overlap = TRUE)+theme_void()
